...
import {
  ...
  GitexAfricaLogo,
  Morocco300Logo
} from './CredibilityLogos';

export const InstitutionalCredibility: React.FC = () => {
  const { t, direction } = useLanguage();

  // 1. Prestigious Recognitions & Awards (Top Achievement Highlights)
  const awards = [
    {
  ...
  const backers = [
    ...
    },
    {
      ...
      Logo: StartupUniverseLogo,
    },
  ];

  // 4. Institutional Recognitions (Row 3 Marquee)
  const recognitions = [
    {
      id: 'al_akhawayn',
      name: t('credibility.recognized_1_name'),
      desc: t('credibility.recognized_1_desc'),
      tag: t('credibility.tag_academic'),
      Logo: AlAkhawaynLogo,
    },
    {
      id: 'uemf',
      name: t('credibility.recognized_2_name'),
      desc: t('credibility.recognized_2_desc'),
      tag: t('credibility.tag_university'),
      Logo: UemfLogo,
    },
    {
      id: 'gitex',
      name: t('credibility.recognized_3_name'),
      desc: t('credibility.recognized_3_desc'),
      tag: t('credibility.tag_government'),
      Logo: GitexAfricaLogo,
    },
    {
      id: 'morocco300',
      name: t('credibility.recognized_4_name'),
      desc: t('credibility.recognized_4_desc'),
      tag: t('credibility.tag_continental'),
      Logo: Morocco300Logo,
    },
  ];
  ...
  const marqueeRecognitions = [...recognitions, ...recognitions, ...recognitions, ...recognitions];
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* ========================================================
            PRESTIGIOUS ACHIEVEMENTS & AWARDS (COMPACT CARDS + LOGOS)
           ======================================================== */}
          ...
          {awards.map((award) => {
            ...
            return (
              <div
                key={award.id}
                className={cn(
                  "relative group/card rounded-2xl p-4 sm:p-4.5 transition-all duration-300 flex flex-col justify-between",
              ...
              >
                <div>
                  ...
                  <h3 className="font-headline text-sm sm:text-base font-extrabold text-slate-900 dark:text-white leading-snug">
                    {award.title}
                  </h3>
                ...
                {/* Bottom Verified Brand Vector Badge */}
                <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-2">
                  <span className="text-[10px] font-headline font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">
                    {t('credibility.verified_laureate')}
                  </span>
                  <div className="h-6 flex items-center">
                    <AwardLogo className="h-5 w-auto max-w-[100px] object-contain opacity-85 group-hover/card:opacity-100 transition-opacity" />
                  </div>
                </div>
              </div>
            );
        ...
        {/* ========================================================
            TRIPLE CONTINUOUS AUTO-SCROLLING HORIZONTAL MARQUEES
            (Supported By, Backed By, Recognized By)
           ======================================================== */}
        <div className="space-y-4">
          
          {/* Marquee Row 1: Supported By (Institutional Supporters) */}
                ...
                {marqueeSupporters.map((item, idx) => {
                  ...
                  return (
                    <div
                      key={`${item.id}-${idx}`}
                      className={cn(
                        "flex items-center gap-3.5 px-4 py-2.5 rounded-2xl transition-all duration-200 flex-shrink-0 cursor-default select-none group/item",
                        "bg-white/85 dark:bg-[#0c2238]/70 backdrop-blur-md",
                        "border border-slate-200/90 dark:border-white/10 hover:border-[#016ba5]/50 dark:hover:border-[#38BDF8]/50 shadow-2xs hover:shadow-md hover:scale-[1.02]",
                        direction === 'rtl' ? 'text-right' : 'text-left'
                      )}
                    >
                      {/* High-fidelity Vector SVG Logo */}
                      <div className="h-7 flex items-center justify-center px-1">
                        <ItemLogo className="h-6 sm:h-7 w-auto max-w-[125px] object-contain" />
                      </div>
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="font-body text-[11px] text-slate-500 dark:text-slate-400 whitespace-nowrap mt-0.5 max-w-[220px] truncate">
                          {item.desc}
                        </span>
                      </div>
                    </div>
          ...
          {/* Marquee Row 2: Backed By (Technology & Venture Ecosystem) */}
          <div className="group relative">
            <div className="flex items-center justify-center gap-2 mb-2 text-xs font-headline font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">
          ...
          {/* Marquee Row 3: Recognized By (Academic & Ministry Bodies) */}
          <div className="group relative">
            <div className="flex items-center justify-center gap-2 mb-2 text-xs font-headline font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">
              <Award className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400" />
              <span>{t('credibility.recognized_heading')}</span>
            </div>

            <div className="relative w-full overflow-hidden mask-marquee-fade py-1" dir="ltr">
              <div className="animate-marquee-slow flex items-center gap-3.5 w-max">
                {marqueeRecognitions.map((item, idx) => {
                  const ItemLogo = item.Logo;
                  return (
                    <div
                      key={`${item.id}-${idx}`}
                      className={cn(
                        "flex items-center gap-3.5 px-4 py-2.5 rounded-2xl transition-all duration-200 flex-shrink-0 cursor-default select-none group/item",
                        "bg-white/85 dark:bg-[#0c2238]/70 backdrop-blur-md",
                        "border border-slate-200/80 dark:border-white/10 hover:border-purple-500/50 dark:hover:border-purple-400/50 shadow-2xs hover:shadow-md hover:scale-[1.02]",
                        direction === 'rtl' ? 'text-right' : 'text-left'
                      )}
                    >
                      {/* High-fidelity Vector SVG Logo */}
